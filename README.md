...
6. Esqueceu de cadastrar um cliente a tempo do lembrete automático (3 dias antes do vencimento)? Abra o cliente e use **"Enviar lembrete agora"** — manda a mesma mensagem de lembrete na hora, manualmente
...
- **Esqueci a senha do painel** — na tela de login, clique em "Esqueci minha senha" e informe a `RECOVERY_KEY` configurada no `.env` (ou nas variáveis de ambiente do Render) junto com a nova senha. Se você já cadastrou a **pergunta secreta** (Config → Ferramentas → "Pergunta secreta"), "Esqueci minha senha" mostra a pergunta primeiro — responda e defina a nova senha; a chave de recuperação continua disponível pelo link "Usar a chave de recuperação". Após 5 respostas erradas seguidas a pergunta trava por 15 minutos.
